...
ascending_genealogy_by_levels = pedigree.get_ascending_graph_from_vertices_by_levels(vertices=[2, 3])
# Alternatively, just get all the vertices in the ascending genealogy
ascending_genealogy = pedigree.get_ascending_vertices_from_probands(probands=[2, 3])
# Perturb the edges
pedigree.remove_edge(parent=6, child=2)
pedigree.add_edge(parent=8, child=2)
# Recalculate the ascending genealogy
modified_ascending_genealogy = pedigree.get_ascending_graph_from_vertices_by_levels(vertices=[2, 3])
